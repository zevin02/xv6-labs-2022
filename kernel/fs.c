...
void ilock(struct inode *ip) // 给inode加锁
{
...
void iunlock(struct inode *ip) // 解锁inode
{
...
void iput(struct inode *ip) // 释放指向inode的指针
{
  acquire(&itable.lock);
...
// returns 0 if out of disk space.
static uint
bmap(struct inode *ip, uint bn) // 返回bn对应的data block number
{
  uint addr, *a;
  uint *a1, *a2;
  struct buf *bp;
  ...
  if (bn < NDOUBLEDIRECT)
  {
    if ((addr = ip->addrs[NSINGLEINDIRECT]) == 0)
    {
      addr = balloc(ip->dev);
      if (addr == 0)
        return 0;
      ip->addrs[NSINGLEINDIRECT] = addr;
    }
    bp = bread(ip->dev, addr); // 获得第一级的映射的数据
    a1 = (uint *)bp->data;
  ...
  panic("bmap: out of range"); // bn块号超过ndirect+nindirect就会崩溃
}
...
// Caller must hold ip->lock.
void stati(struct inode *ip, struct stat *st) // 将inode的元数据拷贝到stat结构体中，该结构体通过stat系统调用展示
{
  st->dev = ip->dev;
  st->ino = ip->inum;
  st->type = ip->type;
...
int readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n) // 从inode里面读取数据
{
  uint tot, m;
  struct buf *bp;
...
//
static char *
skipelem(char *path, char *name) // name就是path中的第一个非/的路径名，而path就是除去name的路径名
{
  char *s;
  int len;
  ...
  while (*path != '/' && *path != 0)
    path++;
  len = path - s; // 低地址-高地址=两者之间的元素个数，为正数
  if (len >= DIRSIZ)
  ...
  while (*path == '/') // 如示例1,path=“bb/c”，name=“a”
    path++;
  return path;
}
...
namex(char *path, int nameiparent, char *name)
{
  ...
  if (*path == '/')
    ...
    ip = idup(myproc()->cwd); // 从当前目录开始计算，并增加引用计数

  while ((path = skipelem(path, name)) != 0)
  {            // 依次考察路径的每个元素，每次迭代都需要在当前节点ip中查找name
    ilock(ip); // 首先给ip上锁
    if (ip->type != T_DIR)
    { // 检查是不是一个目录
      iunlockput(ip);
...
// 路径名查询
struct inode *
namei(char *path) // 计算path并返回对应的inode，返回path的inode
{
  char name[DIRSIZ];
...
struct inode *
nameiparent(char *path, char *name) // 在最后一个元素之前停止，返回父目录的inode，将最后一个元素复制到name中，返回path的parent的inode
{
